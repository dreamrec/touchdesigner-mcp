...
async def td_list_families(ctx: Context) -> str:
    """List all operator families (TOP, CHOP, SOP, POP, DAT, COMP, MAT) and the specific
    node types present in the current project. Useful for discovering what operators
    are available before creating new nodes. POPs are the GPU-accelerated point operators
    introduced in TouchDesigner 2025 for particles, point clouds, and geometry.
...
async def td_create_node(params: CreateNodeInput, ctx: Context) -> str:
    ...
    - TOPs: noiseTOP, levelTOP, nullTOP, compositeTOP, feedbackTOP, moviefileinTOP, renderTOP, simpleRenderTOP, layerMixTOP
    - CHOPs: waveCHOP, noiseCHOP, nullCHOP, mathCHOP, constantCHOP, selectCHOP, mergeCHOP, timerCHOP, laserCHOP
    - SOPs: sphereSOP, boxSOP, gridSOP, lineSOP, nullSOP, transformSOP, noiseSOP, mergeSOP, particleSOP
    - POPs (TD 2025+, GPU-accelerated): noisePOP, transformPOP, particlePOP, feedbackPOP, renderPOP, linePOP, spritePOP, convertPOP, mergePOP, nullPOP, switchPOP, DMXFixturePOP, DMXOutPOP
    - DATs: textDAT, tableDAT, scriptDAT, nullDAT, selectDAT, chopexecDAT, webserverDAT, DMXMapDAT
    - COMPs: baseCOMP, containerCOMP, geometryCOMP, cameraCOMP, lightCOMP, geoTextCOMP
